...
export async function handleGreenApiWebhook(body: any) {
  ...
  if (typeWebhook === 'incomingMessageReceived') {
    const tenant = await getTenantByInstance(body.instanceId)
    ...
    if (tenant?.canReceive) {
      ...
      // Callback ל-tenant (אם הגדיר URL)
      if (tenant.incomingWebhookUrl) {
        await fetch(tenant.incomingWebhookUrl, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ type: 'incoming', message: incoming })
        })
      }
    }
  }
...
### 1. MessageQueue - שליחה עם Throttle
```typescript
// shared/services/message-queue.service.ts
export class MessageQueueService {
  private static queue: QueuedMessage[] = []
  private static processing = false
  private static MESSAGES_PER_SECOND = 10  // לפי הגבלת Green-API

  static async add(message: QueuedMessage): Promise<string> {
    const queueId = generateUUID()
    this.queue.push({ ...message, queueId })
  ...
  private static async process() {
    ...
    while (this.queue.length > 0) {
      // לקחת batch לפי הגבלת הספק
      const batch = this.queue.splice(0, this.MESSAGES_PER_SECOND)

      // לשלוח במקביל
      await Promise.all(batch.map(m => this.sendOne(m)))

      // להמתין שנייה לפני ה-batch הבא
  ...
  private static async sendOne(message: QueuedMessage): Promise<void> {
    try {
      ...
      await this.updateMessageStatus(message.id, { success: false, error })
    }
  }
}
```
